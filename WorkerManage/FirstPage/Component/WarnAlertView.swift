...
class WarnAlertView: UIView {
    ...
    lazy var entryAttributes: EKAttributes = {
        ...
        attributes.name = "DeviceWarn"
        attributes.positionConstraints.size = .init(width: .constant(value: 260), height: .intrinsic)
        attributes.roundCorners = .all(radius: 6)
        attributes.positionConstraints.safeArea = .empty(fillSafeArea: true)
        attributes.windowLevel = .normal
        attributes.screenBackground = .color(color: EKColor(.blackColor_25))
        attributes.entryBackground = .color(color: EKColor(.whiteColor))
    ...
    func dismiss() {
        SwiftEntryKit.dismiss(.specific(entryName: "DeviceWarn"))
    }
